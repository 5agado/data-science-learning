...
class UtilsTest(unittest.TestCase):
    def test_binToColor(self):
        # Blue
        color = utils.bin_to_color(np.array([0,0,0,0,0,0,1,1,1]))
        self.assertEqual(color, (0., 0., 1.0))
        # Red
        color = utils.bin_to_color(np.array([1,1,1,0,0,0,0,0,0]))
        self.assertEqual(color, (1., 0., 0.))
        # White
        color = utils.bin_to_color(np.array([1,1,1,1,1,1,1,1,1]))
        self.assertEqual(color, (1., 1., 1.))
        # White uneven length (moved to 2BPC, and last two bits are discarded
        color = utils.bin_to_color(np.array([1,1,1,1,1,1,0,1]))
        self.assertEqual(color, (1., 1., 1.))
        # Insufficient len, defaults to gray
        color = utils.bin_to_color(np.array([1,1]))
